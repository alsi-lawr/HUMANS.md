...
optional hardening follow-up without blocking acceptance. Stop the batch for human resolution when
a finding expands architecture, durable state, dependencies, failure guarantees, compatibility,
public behavior, or material paths, or when the same concern survives one correction. Complete each
