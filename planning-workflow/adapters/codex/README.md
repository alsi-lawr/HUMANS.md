...
2. Copy role TOMLs directly (not as symlinks) to `$CODEX_HOME/agents/`.
3. Copy the nine workflow skill directories directly to `$CODEX_HOME/skills/`.
4. Merge only the named `[agents.<role>]` declarations and compatible `[agents]` limits from `config-fragment.toml`; preserve unrelated config.
5. Run strict Codex configuration validation in a fresh process and rerun the validator with `--codex-home` to prove installed-copy parity.
