#!/usr/bin/env python3
"""Validate the three explicit v0.2.2 package roots and catalog inventory."""
...
def main() -> int:
    ...
    if versions != {"0.2.2"}: errors.append(f"package versions not synchronized: {sorted(versions)}")
    ...
    for vendor, marker in (("codex", ".codex-plugin/plugin.json"), ("claude", ".claude-plugin/plugin.json")):
        ...
        if generated.exists():
            ...
            for name in EXPECTED & identities:
                ...
                if metadata.get("name") != name or metadata.get("version") != "0.2.2": errors.append(f"generated {vendor} metadata mismatch: {name}")
