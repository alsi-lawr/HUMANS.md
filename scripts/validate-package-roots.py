#!/usr/bin/env python3
"""Validate the three explicit package roots and catalog inventory."""
...
def main() -> int:
    manifests=sorted(ROOT.glob("*/packaging/plugin.toml")); errors=[]; names=set(); versions={}
    for path in manifests:
        item=tomllib.loads(path.read_text(encoding="ascii")); name=item.get("name"); names.add(name); versions[name]=item.get("version")
    ...
    if len(set(versions.values())) != 1: errors.append(f"package versions not synchronized: {sorted(map(str, versions.values()))}")
    ...
    for vendor, marker in (("codex", ".codex-plugin/plugin.json"), ("claude", ".claude-plugin/plugin.json")):
        ...
        if generated.exists():
            ...
            for name in EXPECTED & identities:
                ...
                if metadata.get("name") != name or metadata.get("version") != versions.get(name): errors.append(f"generated {vendor} metadata mismatch: {name}")
